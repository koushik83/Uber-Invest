...
def get_13f_filings(cik: str, num_quarters: int = BACKFILL_QUARTERS) -> list[dict]:
    ...
    *num_quarters* 13F-HR filings, oldest-first.
    ...
    # Cutoff: skip pre-2020 filings -- they use an older XML format our parser
    # cannot handle, and they raise RuntimeError which kills the backfill run.
    MIN_REPORT_DATE = "2020-01-01"

    results = []
    for form, acc, rdate, fdate in zip(forms, accessions, report_dates, filing_dates):
        if form == "13F-HR" and rdate >= MIN_REPORT_DATE:
            results.append({
